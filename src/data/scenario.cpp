...
Scenario::Scenario()
    : m_scenarioId(QUuid::createUuid().toString())
    , m_duration(60)
    , m_level(BEGINNER)
    , m_maximumPlayers(6)
    , m_minimumPlayers(2)
...
void Scenario::setReferenceScenario(Scenario* a)
{
    if(a == nullptr)
        return;
...
void Scenario::playedOnceMore()
{
    ++m_playedCount;
}

void Scenario::playedOnceLess()
{
    --m_playedCount;
}

quint32 Scenario::getPlayedCount()
{
...
void Scenario::readDataFromJson(QJsonObject& obj)
{
    ...
    m_playedCount= obj["playedCount"].toInt(0);
}
...
void Scenario::writeDataToJson(QJsonObject& obj)
{
    ...
    obj["playedCount"]= static_cast<int>(m_playedCount);
}

void Scenario::writeDataToCsv(QTextStream& out)
{
    out << m_gameMasterId << ";" << m_gameMasterId << ";" << m_gameId << ";" << m_duration << ";" << m_level << ";"
        << m_maximumPlayers << ";" << m_minimumPlayers << ";" << m_title << ";" << m_description << ";"
        << m_currentPlayers << ";" << m_state << ";" << m_startTime.toString("dd.MM.yyyy:hh:mm:ss") << "\n";
}
