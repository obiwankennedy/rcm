...
Schedules::Schedules(qreal x, qreal y, qreal w, qreal h) : QGraphicsScene(x, y, w, h)
{
    m_timer= new QTimer();
...
void Schedules::readDataFromJson(QJsonObject& obj)
{
    ...
    m_minuteWidth= obj["minuteWidth"].toDouble();
...
void LocalisationView::readDataFromJson(QJsonObject& obj)
{
    ...
    for(it= array.begin(); it != array.end(); ++it)
    {
        ...
        auto schedule= new Schedules(0, 0, SCHEDULE_WIDTH, 900);
        ...
        if(0 < schedule->getNumberOfTable())
        {
            ...
            schedule->setSceneRect(0, 0, SCHEDULE_WIDTH * 1.2, htotal);
        }
    }
...
void LocalisationView::contextMenuOnView(QPoint f)
{
    ...
    auto selectedAction= menu.exec(m_view->mapToGlobal(f));
    ...
    if(nullptr != selectedItem)
    {
        ...
        if(selectedAction == m_addPlayer)
        {
            ...
            if(QDialog::Accepted == dialog.exec())
            {
                ...
                sce->addPlayerInfo(list.join(';'));
                sce->increaseCurrentPlayerCount(static_cast<quint32>(count));
            }
        }
...
void LocalisationView::setProperties()
{
    ...
    if(QDialog::Accepted == wizzard.exec())
    {
        ...
        {
            auto h= TABLE_HEIGHT;
            Schedules* scene= new Schedules(0, 0, SCHEDULE_WIDTH * 1.2, h * tableCount);
            if(0 < scene->getNumberOfTable())
            {
                ...
                scene->setSceneRect(0, 0, SCHEDULE_WIDTH * 1.2, htotal);
            }
